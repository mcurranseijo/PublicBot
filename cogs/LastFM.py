...
class MusicCog(commands.Cog):
    ...
    @commands.command(
        name="getsong"
        brief="A cog to get info about a song, takes in song name or song name and artist, formatting for the second is songname, artist with a comma seperating the two."
    )
    ...
    async def topsongs(self,ctx):
      ...
      for (count, f) in enumerate(data[:10], start=1):
